...
IMG_DIR_SRC, IMG_DIR_TAR = "/root/dataset/mnist", "/root/dataset/mnist_m/"
IMAGE_SIZE = 28
BATCH_SIZE = 128
...
def load_data(src_root=IMG_DIR_SRC, tar_root=IMG_DIR_TAR, batch_size=128):
    ...
    dataset_source = datasets.MNIST(
        root=src_root, train=True, transform=img_transform, download=True
    )
...
def load_test_data(dataset_name, batch_size=128):
    if dataset_name == "mnist_m":
        test_list = "/root/dataset/mnist_m/mnist_m_test_labels.txt"
        dataset = GetLoader(
            data_root="/root/dataset/mnist_m/mnist_m_test",
            data_list=test_list,
            transform=img_transform,
        )
    else:
        dataset = datasets.MNIST(
            root=IMG_DIR_SRC, train=False, transform=img_transform, download=True
        )
